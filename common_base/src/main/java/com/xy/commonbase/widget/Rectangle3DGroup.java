package com.xy.commonbase.widget;
...
public class Rectangle3DGroup extends ViewGroup {
    ...
    public Rectangle3DGroup(Context context) {
        this(context, null);
    }

    public Rectangle3DGroup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Rectangle3DGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                child.layout(params.leftMargin, childTop,
                        child.getMeasuredWidth() + params.rightMargin, childTop + child.getMeasuredHeight());
...
//        requestDisallowInterceptTouchEvent(true);
     ...
     *
     * @param canvas
     * @param screen      当前的子View Position
     * @param drawingTime
     */
    ...
    public void initAnim() {
        ...
        disposable = Observable.interval(3, TimeUnit.SECONDS)
                .filter(a -> mHeight > 0)
                .filter(aLong -> isAttachedToWindow())
...
//                    animator.start();
    ...
    public Disposable removeDisposable() {
        return disposable;
    }
