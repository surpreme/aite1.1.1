...
import com.xy.commonbase.http.RetrofitClient;
...
//            startActivity(Intent.createChooser(sendIntent,"分享到..."));
    ...
    interface ApiService{
        @POST("{port}/index.php?act=member_index&op=getH5AccessToken")
        @FormUrlEncoded
        Observable<ResponseBody> getToekn(@Path("port") String port,@Field("key") String token);
    }
    ...
    protected void initView() {
        disposable.add(RetrofitClient.getInstance().getRetrofit().create(ApiService.class).getToekn("mobile",BaseApplication.token)
                .map(ResponseBody::string)
                .compose(RxUtils.rxSchedulerHelper())
                .subscribe(this::setView));
    }

    private void setView(String  s) throws JSONException {
        JSONObject object = new JSONObject(s);
        JSONObject datas = object.optJSONObject("datas");
        token = datas.optString("access_token");
        url = url+token;
        if (url.contains("?")) {
...
//        token = MyApp.getH5TOKEN();
        LogHelper.e("---------------------", "链接   " + url + "====h5token" + token);
        // 声明WebSettings子类
        clearCookies(this);
...
//                    }
            ...
            public void showDiaLogHelperHelper() {
                if (ip == null) {
                    ip = new ImagePick(BaseWebActivity.this);
                }
...
//        }
    ...
    class AndroidWebView {
        ...
        public void AppShare(String title,String desc,String link,String img){
            LogHelper.e("-------->"+title+"\n"+desc+"\n"+link+"\n"+img);
            runOnUiThread(()->{
                UMImage thumb = new UMImage(BaseWebActivity.this,img);
